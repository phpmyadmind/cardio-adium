...
function StatisticsContent() {
  ...
  // Si tiene acceso QR, permitir acceso sin validación de usuario
  // Solo validar si NO tiene acceso QR y NO es admin
  useEffect(() => {
    if (!isUserLoading && !hasQrAccess && !user?.isAdmin) {
      router.replace("/login");
    }
  }, [hasQrAccess, isUserLoading, user, router]);

  if (!hasQrAccess && !user?.isAdmin && !isUserLoading) {
    return (
          ...
          <div className="flex items-center gap-2">
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Cerrar sesión" disabled={isLoggingOut}>
