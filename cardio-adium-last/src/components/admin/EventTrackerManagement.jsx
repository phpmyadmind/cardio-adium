...
export function EventTrackerManagement() {
  ...
  const [editingEventTracker, setEditingEventTracker] = useState(null);
  const { data: eventTrackers = [], isLoading, refetch } = useApiCollection('/api/event-trackers');
  ...
  const handleOpenDialog = (eventTracker) => {
    if (eventTracker) {
  ...
  const onSubmit = async (data) => {
    try {
      const url = editingEventTracker
        ? getApiUrl(`/api/event-trackers/${editingEventTracker.id}`)
        : getApiUrl('/api/event-trackers');
      const method = editingEventTracker ? 'PUT' : 'POST';
      const body = editingEventTracker
        ? { name: data.name, description: data.description || undefined, isActive: data.isActive }
        : { name: data.name, description: data.description || undefined, isActive: data.isActive };
  ...
  const handleDelete = async (id, name) => {
    ...
    if (!confirm(`¿Está seguro de que desea eliminar el event tracker "${name}"?`)) return;

    try {
      const response = await fetch(getApiUrl(`/api/event-trackers/${id}`), { method: 'DELETE' });
  ...
  const handleToggleActive = async (eventTracker) => {
    try {
      const response = await fetch(getApiUrl(`/api/event-trackers/${eventTracker.id}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: eventTracker.name,
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>Gestión de Event Trackers</CardTitle>
          <CardDescription>
            Administra los eventos principales del sistema. El evento activo será asignado por defecto a los nuevos usuarios.
          </CardDescription>
        </div>
