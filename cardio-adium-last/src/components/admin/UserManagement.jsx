...
export function UserManagement() {
  const { data: users = [], isLoading: usersLoading, refetch } = useApiCollection('/api/users');
  ...
  const form = useForm({
    ...
    defaultValues: { name: '', event_tracker: 'none' },
  });
  ...
  const handleOpenDialog = (user) => {
    ...
    form.reset({
      name: user.name,
      event_tracker: user.event_tracker || user.eventTracker || 'none',
    });
    setDialogOpen(true);
  };
        ...
        <Dialog open={isDialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Modifique el nombre y el evento tracker del usuario.</DialogDescription>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleUpdate)} className="space-y-4">
                ...
                <DialogFooter>
                  ...
                  <Button type="submit" className="h-12 text-base font-bold rounded-xl bg-[#2E61FA] hover:bg-[#365899] text-white shadow-md">
                    Actualizar Usuario
                  </Button>
                </DialogFooter>
              </form>
            ...
            {regularUsers.length === 0 ? (
              ...
              regularUsers.map((user) => (
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    {user.event_tracker || user.eventTracker
                      ? eventTrackerMap.get(user.event_tracker || user.eventTracker) || user.event_tracker || user.eventTracker
                      : '-'}
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
